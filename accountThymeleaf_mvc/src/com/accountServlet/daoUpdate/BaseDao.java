...
public abstract class BaseDao<T> {
    ...
    public int updateWithoutConn(Connection conn, String sql, Object... args) {
...
//            set parameters
            ...
            try {
                conn.setAutoCommit(true);//recover this to default true, avoid conflict during using pool
            } catch (SQLException e) {
                e.printStackTrace();
            }
            JDBCutileWithPool.close(ps, null);
        }
        return 0;
    }

    public T queryInstanceWithoutConn(Connection conn, String sql, Object... args) {
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ps = conn.prepareStatement(sql);
...
//                    using reflect to assign the value to the property
            ...
            JDBCutileWithPool.close(rs, ps, null);
        }
        return null;
    }
